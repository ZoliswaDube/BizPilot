/*
  # Complete BizPilot Database Schema with Robust User Management

  1. New Tables
    - `categories` - Product categories with hierarchical support
    - `suppliers` - Supplier information management
    - `inventory_transactions` - Track inventory changes
    - `ai_conversations` - AI chat conversation history
    - `ai_messages` - Individual messages in conversations
    - `qr_codes` - QR code generation for tip pages

  2. Enhanced Tables
    - Add new columns to `products` table (sku, location, supplier_id, etc.)
    - Add new columns to `inventory` table (product_id, batch_lot_number, etc.)

  3. Robust User Management
    - Ultra-robust user profile creation with UPSERT operations
...
-- Drop existing auth triggers to recreate them with ultra-robust functions
...
-- Ultra-robust function to handle user updates with UPSERT
...
BEGIN
    ...
    -- Extract and validate email - provide fallback
    user_email := COALESCE(NEW.email, 'user-' || NEW.id::text || '@temp.local');
    ...
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'handle_user_update: Error extracting metadata for user %: %', NEW.id, SQLERRM;
    END;
    ...
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'handle_user_update: Error extracting provider for user %: %', NEW.id, SQLERRM;
        user_provider := 'email';
    END;
    ...
    BEGIN
        ...
        RAISE LOG 'handle_user_update: Successfully upserted profile for user %', NEW.id;
        
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'handle_user_update: Failed to upsert profile for user %: % (SQLSTATE: %)', NEW.id, SQLERRM, SQLSTATE;
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'handle_user_update: Failed to create settings for user %: %', NEW.id, SQLERRM;
    END;
...
-- Recreate triggers with the ultra-robust functions
