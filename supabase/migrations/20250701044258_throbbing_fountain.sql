/*
  # Enhanced Database Schema - Categories, Suppliers, Transactions, AI Chat, and QR Codes

  1. New Tables
    - `categories` - Product categories with hierarchical support
    - `suppliers` - Supplier management
    - `inventory_transactions` - Track all inventory changes
    - `ai_conversations` - Store AI chat sessions
    - `ai_messages` - Store individual chat messages
    - `qr_codes` - Store QR code configurations
...
BEGIN
  ...
  -- Add foreign key constraint for inventory user_id if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
